...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    if (!PyObject_TypeCheck(args, &Matrix61cType)) {
        PyErr_SetString(PyExc_TypeError, "args is not of type Matrix61c");
    }
    Matrix61c* argMatrix = (Matrix61c*) args;
    Matrix61c* result = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    matrix *new_mat;
    int new_mat_failed = allocate_matrix(&new_mat, self->mat->rows, self->mat->cols);
    if (new_mat_failed != 0) {
        if (new_mat_failed == -1) {
            PyErr_SetString(PyExc_ValueError, "Tried to allocate matrix with non-positive dimensions");
        }
        else {
            PyErr_SetString(PyExc_RuntimeError, "Failed to allocate new matrix");
        }
        return NULL;
    }
    result->mat = new_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(new_mat->cols));
    int ret = add_matrix(result->mat, self->mat, argMatrix->mat);
    if (ret != 0) {
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    result->mat = new_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(new_mat->cols));
    int ret = sub_matrix(result->mat, self->mat, argMatrix->mat);
    if (ret != 0) {
        PyErr_SetString(PyExc_ValueError, "a and b do not have the same dimensions.");
    }
    return (PyObject*) result;
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    Matrix61c* argMatrix = (Matrix61c*) args;
	Matrix61c* result = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    // Matrix61c* result = (Matrix61c*) Matrix61c_init(PyTuple_Pack(3, self->mat->rows, argMatrix->mat->rows, 0));
    matrix *new_mat;
    int new_mat_failed = allocate_matrix(&new_mat, self->mat->rows, argMatrix->mat->cols);
    if (new_mat_failed != 0) {
        if (new_mat_failed == -1) {
            PyErr_SetString(PyExc_ValueError, "Tried to allocate matrix with non-positive dimensions");
        }
    ...
    result->mat = new_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(new_mat->cols));
    int mul_failed = mul_matrix(result->mat, self->mat, argMatrix->mat);
	if (mul_failed != 0) {
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    result->mat = new_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(new_mat->cols));
    int ret = pow_matrix(new_mat, self->mat, p);
    if (ret != 0) {
        if (ret == -3) {
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 3, 3, &row, &col, &val)) {
        if (row && col && val && PyLong_Check(row) && PyLong_Check(col) && (PyLong_Check(val) || PyFloat_Check(val))) {
            int r = PyLong_AsLong(row), c = PyLong_AsLong(col);
            if (r >= 0 && c >= 0 && r < self->mat->rows && c < self->mat->cols) {
                set(self->mat, r, c, PyFloat_AsDouble(val));
                return Py_BuildValue("");
            }
            else {
                PyErr_SetString(PyExc_IndexError, "Invalid row or col");
                return NULL;
            }
        }
        else {
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 2, 2, &row, &col)) {
        if (row && col && PyLong_Check(row) && PyLong_Check(col)) {
            int r = PyLong_AsLong(row), c = PyLong_AsLong(col);
            if (r >= 0 && c >= 0 && r < self->mat->rows && c < self->mat->cols) {
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"get", (PyCFunction) Matrix61c_get_value, METH_VARARGS, "returns the value at the `row`th row and `col`th column as a float"},
    {"set", (PyCFunction) Matrix61c_set_value, METH_VARARGS, "sets the value at the `row`th row and `col`th column to 'val'"},
    {NULL, NULL, 0, NULL}
};
